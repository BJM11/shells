...
if(isset($p['mailFrom'])&&isset($p['mailTo'])&&isset($p['mailSubject'])&&isset($p['mailContent'])){
	...
	$mailAttachment = (!empty($mailAttachment))? explode("{[|RM|]}", $p['mailAttachment']):array();
...
<link rel='SHORTCUT ICON' href='<?php echo get_resource('RM');?>'>
<style type="text/css">
@font-face {
...
<div id='wrapper'>
	...
	<div id='header'>
		...
		<div id='headerNav'>
			...
			<img onclick='viewfileorfolder();' id='RM' src='<?php echo get_resource('RM');?>' /><span id='nav'>&nbsp;<?php echo $nav; ?></span>
...
function mail_send(){
	...
	mailAttachment = '';
	if($('.mailAttachment')){
		mailAttachment = $('.mailAttachment').map(function(){ return this.value; }).get().join('{[|RM|]}');
	}
